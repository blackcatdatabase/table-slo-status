...
namespace BlackCat\Database\Packages\SloStatus\Joins;
...
final class SloStatusJoins {
    ...
    /**
     * FK: slo_status -> slo_windows
     * LEFT JOIN vw_slo_windows AS $as ON $as.id = $alias.window_id
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinSloWindows(string $alias = 't', string $as = 'j0'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_slo_windows AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.window_id' . ' ', []];
    }
