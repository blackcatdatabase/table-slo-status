...
use BlackCat\Database\Packages\SloStatus\Repository\SloStatusRepositoryInterface;
...
class SloStatusRepository implements SloStatusRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'window_id', 'computed_at' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\SloStatus\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'sli_value', 'good_events', 'total_events', 'status' ]);
          return count($rows);
        }
    ...
    public function getByWindowIdAndComputedAt(int $windowId, \DateTimeInterface|string $computedAt, bool $asDto = false): array|\BlackCat\Database\Packages\SloStatus\Dto\SloStatusDto|null {
        return $this->getByUnique([ 'window_id' => $windowId, 'computed_at' => $computedAt ], $asDto);
    }
    ...
    public function getIdByWindowIdAndComputedAt(int $windowId, \DateTimeInterface|string $computedAt) {
        $row = $this->getByWindowIdAndComputedAt($windowId, $computedAt, false);
