...
namespace BlackCat\Database\Packages\SloStatus\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\SloStatus\Definitions;
use BlackCat\Database\Packages\SloStatus\Criteria;
use BlackCat\Database\Packages\SloStatus\Dto\SloStatusDto as Dto;
use BlackCat\Database\Packages\SloStatus\Mapper\SloStatusDtoMapper as RowMapper;
...
final class SloStatusRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\SloStatus\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'sli_value', 'good_events', 'total_events', 'status' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'window_id', 'computed_at' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\SloStatus\Definitions::table());
            return $bulk->upsertMany($rows, [ 'window_id', 'computed_at' ], [ 'sli_value', 'good_events', 'total_events', 'status' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\SloStatus\Dto\SloStatusDto|null */
    public function getByWindowIdAndComputedAt(int $windowId, \DateTimeInterface|string $computedAt, bool $asDto = false): array|\BlackCat\Database\Packages\SloStatus\Dto\SloStatusDto|null {
        $row = $this->getByUnique([ 'window_id' => $windowId, 'computed_at' => $computedAt ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\SloStatus\Mapper\SloStatusDtoMapper::fromRow($row);
    }
    public function existsByWindowIdAndComputedAt(int $windowId, \DateTimeInterface|string $computedAt): bool {
        $where = 't.' . Ident::q($this->db, 'window_id') . ' = :uniq_window_id' . ' AND ' . 't.' . Ident::q($this->db, 'computed_at') . ' = :uniq_computed_at';
        return $this->exists($where, [ 'uniq_window_id' => $windowId, 'uniq_computed_at' => $computedAt ]);
    }
    /** @return int|string|null */
    public function getIdByWindowIdAndComputedAt(int $windowId, \DateTimeInterface|string $computedAt) {
        $row = $this->getByWindowIdAndComputedAt($windowId, $computedAt);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\SloStatus\Dto\SloStatusDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\SloStatus\Dto\SloStatusDto|null {
