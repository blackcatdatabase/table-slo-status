...
namespace BlackCat\Database\Packages\SloStatus\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\SloStatus\Dto\SloStatusDto;
use BlackCat\Database\Packages\SloStatus\Mapper\SloStatusDtoMapper;
 ...
 */
final class SloStatusAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private SloStatusRepository $sloStatusRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'slo_status' => $this->sloStatusRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\SloStatus\ModuleException("Record {$idStr} not found for locking.");
            }
